...
function getRelativePath (
  ...
  if (oriPath.indexOf('.') !== 0) {
    ...
    if (!fs.existsSync(testIndex)) { // 如果不在
    ...
    const npmIndex = path.join(rootPath, `miniprogram_npm/${oriPath}.js`) // 判断本身是否是一个完整的入口
    if (fs.existsSync(npmIndex)) {
      const realPath = getModulePath(rootPath, oriPath)
      // 转成相对路径
      let relativePath = realPath.replace(rootPath + '\\node_modules\\', '')
...
export function analyzeImportUrl (
  ...
  } else if (fs.existsSync(path.join(rootPath, 'node_modules', value))) {
